// Não esquecer de chamar initFFT
...
std::vector<int> mul(const std::vector<int> &a, const std::vector<int> &b) {
  ...
  //Not excluding the zero coeficients in the end
  while(c.size() > 0 && c.size() > a.size() + b.size() - 1) c.pop_back();
  
  //Excluding the zero coeficients in the end
